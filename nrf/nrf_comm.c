...
nrf_pck_wrapper nrf_tx_wrapper;
nrf_pck_wrapper nrf_rx_wrapper;
...
DEFINE_KFIFO(nrf_tx_fifo, nrf_pck_wrapper, NRF_COMM_TX_PCK_FIFO_SIZE);
DEFINE_KFIFO(nrf_rx_fifo, nrf_pck_wrapper, NRF_COMM_RX_PCK_FIFO_SIZE);
...
static void nrf_comm_cmd(nrf_pck_wrapper *wrapper);
...
/**
 * @brief	NRF 基于协议非阻塞式发送函数
 * @param   data        发送数据
 * @param   len         数据长度
 * @param   refer       发送数据来源 NRF_UART / NRF_CAN / NRF_MCU
 * @param   host        发送数据处理后输出方 NRF_UART / NRF_CAN / NRF_MCU 组合
 *          e.g. NRF_UART | NRF_CAN
 */
void nrf_comm_send(uint8_t *data, uint32_t len, uint8_t refer, uint8_t host) {
    NRF_COMM_SET_PCK_REFER_HOST(&nrf_tx_wrapper.pck, refer, host);
    nrf_tx_cnt = len/(32 - NRF_PCK_HEADER_SIZE);
    nrf_tx_remainder = len%(32 - NRF_PCK_HEADER_SIZE);
    for (cnt = 0; cnt < nrf_tx_cnt; cnt++) {
        NRF_COMM_SET_PCK_CNT_INDEX(&nrf_tx_wrapper.pck, nrf_tx_cnt, cnt);
        memcpy(nrf_tx_wrapper.pck.pck.data, data + NRF_PCK_SIZE*cnt, NRF_PCK_SIZE);
        nrf_tx_wrapper.len = 32;
        kfifo_put(&nrf_tx_fifo, nrf_tx_wrapper);
    }
    NRF_COMM_SET_PCK_CNT_INDEX(&nrf_tx_wrapper.pck, nrf_tx_cnt, cnt);
    memcpy(nrf_tx_wrapper.pck.pck.data, data + NRF_PCK_SIZE*cnt, nrf_tx_remainder);
    nrf_tx_wrapper.len = nrf_tx_remainder;
    kfifo_put(&nrf_tx_fifo, nrf_tx_wrapper);
...
void nrf_comm_block_send(uint8_t *data, uint32_t len, uint8_t refer, uint8_t host) {
    ...
    for (cnt = 0; cnt < nrf_tx_cnt; cnt++) {
        ...
        nrf_send_data(nrf_tx_wrapper.pck.ui8, 32);
        while(_nrf_lock); // unlock in gpio exti nrf_irq_handle
    }
    nrf_lock();
    NRF_COMM_SET_PCK_CNT_INDEX(&nrf_tx_wrapper.pck, nrf_tx_cnt, cnt);
    memcpy(nrf_tx_wrapper.pck.pck.data, data + NRF_PCK_SIZE*cnt, nrf_tx_remainder);
    // _nrf_comm_send(nrf_tx_data, NRF_PCK_HEADER_SIZE + nrf_tx_remainder);
    nrf_send_data(nrf_tx_wrapper.pck.ui8, NRF_PCK_HEADER_SIZE + nrf_tx_remainder);
    while(_nrf_lock); // unlock in gpio exti nrf_handle
}
...
void nrf_main(void) {
    ...
    case NRF_COMM_SEND:
        ...
        nrf_comm_block_send(nrf_tx_buffer, nrf_handle.tx_len, nrf_handle.nrf_data_from, nrf_handle.nrf_data_to);
    ...
    while(!kfifo_is_empty(&nrf_rx_fifo)) {
        kfifo_get(&nrf_rx_fifo, &nrf_rx_wrapper);
        _nrf_receive_callback(nrf_rx_wrapper.pck.ui8, nrf_rx_wrapper.len);
    }
}
...
void nrf_irq_handle(void) {
	...
	if (NRF_STATUS_GET_RX_DR(status)) {
		...
		_nrf_clear_tx_irq();
		_nrf_flush_tx();
        if (_nrf_fifo_send_flag && !kfifo_is_empty(&nrf_tx_fifo)) {
            nrf_fifo_send();
        } else {
...
static void _nrf_receive_callback(uint8_t *data, int len) {
    ...
    if (deal_method & NRF_UART) {
        ...
        HAL_UART_Transmit_DMA(pCMD_USART, data + NRF_PCK_HEADER_SIZE, len - NRF_PCK_HEADER_SIZE);
    ...
    #endif // IND_LED_Pin
    nrf_receive_callback(nrf_rx_wrapper.pck.ui8, nrf_rx_wrapper.len);
}

#ifdef SL_NRF_COMM_CMD
static void nrf_comm_cmd(nrf_pck_wrapper *wrapper) {
    uint8_t arg = wrapper->pck.pck.data[0]&0x0F;
    uint8_t cmd = wrapper->pck.pck.data[0]>>4;
    ...
    case NRF_COMM_CMD_PING:
        nrf_comm_block_send((uint8_t*)str_tmp, 10, NRF_MCU, NRF_UART);
        HAL_GPIO_TogglePin(IND_LED_GPIO_Port, IND_LED_Pin);
        break;
...
static inline void nrf_fifo_send(void) {
    kfifo_get(&nrf_tx_fifo, &nrf_tx_wrapper);
    nrf_send_data(nrf_tx_wrapper.pck.ui8, nrf_tx_wrapper.len);
}
